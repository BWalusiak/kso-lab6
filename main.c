...
void print_help() {
    ...
    printf("--put-file <filesystem> <file>: copy the file from the current directory to the filesystem\n");
...
int main(int argc, char *argv[]) {
    ...
    if (strcmp(argv[1], "--delete-file") == 0) {
        ...
        // delete file data
        memset(filesystem + FILE_DATA_OFFSET + file_descriptor->file_offset, 0, file_descriptor->file_size);
        // move all other file data to the left
        memmove(filesystem + FILE_DATA_OFFSET + file_descriptor->file_offset,
                filesystem + FILE_DATA_OFFSET + file_descriptor->file_offset + file_descriptor->file_size,
                MAX_FILESYSTEM_SIZE - (FILE_DATA_OFFSET + file_descriptor->file_offset + file_descriptor->file_size));
